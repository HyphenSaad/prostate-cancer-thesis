...
class ResNetBaseline(nn.Module):
  def __init__(self, block, layers):
    ...
    self.layer4 = self._make_layer(block, 512, layers[3], stride=2) # TAG: UPDATED_OTHER_THAN_GPFM_TOOLKIT_CODE
  ...
  def forward(self, x):
    ...
    x = self.layer2(x)
    x = self.layer3(x)
    x = self.layer4(x) # TAG: UPDATED_OTHER_THAN_GPFM_TOOLKIT_CODE
