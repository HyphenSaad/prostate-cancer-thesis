...
!echo -e "[B{BATCH_NUMBER}-S4] Downloading Create Patches Files..."
...
!echo -e "\n[B{BATCH_NUMBER}-S4] Unzipping Create Patches Files..."
!mkdir output_{BATCH_NUMBER}
!unzip -q output_{BATCH_NUMBER}_create_patches.zip -d output_{BATCH_NUMBER}

!echo -e "[B{BATCH_NUMBER}-S4] Downloading Extract Patches Files..."
...
!echo -e "\n[B{BATCH_NUMBER}-S4] Unzipping Extract Patches Files..."
!unzip -q output_{BATCH_NUMBER}_extract_patches.zip -d output_{BATCH_NUMBER}

!echo -e "\n[B{BATCH_NUMBER}-S4] Deleting Extra Files..."
...
!echo -e "\n[B{BATCH_NUMBER}-S4] Creating ZIP File For Batch # {BATCH_NUMBER}"
...
!echo -e "\n[B{BATCH_NUMBER}-S4] Removing Directory For Batch # {BATCH_NUMBER}"
!rm -r "output_{BATCH_NUMBER}"; echo "Directory Removed For Batch # {BATCH_NUMBER}"

!echo -e "\n[B{BATCH_NUMBER}-S4] All Done!"