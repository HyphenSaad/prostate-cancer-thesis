BATCH_NUMBER = 0

ACCESS_TOKEN = ""
ENCODER = "ctranspath"

!rm -rf output_{BATCH_NUMBER}.zip

S3_CREATE_PATCHES = ['12wPf9U0kLygUdPsqhneVJBpJ0Uy0SMi2', '13iZVOBUu7Y-v-hqy4wTLnmCBQ-Evwvbw', '14Jx6filtfYYTZsIlijN_a8rcVteGkhRZ', '15-baV9dQ9Dia2gMVSwm3u2imuo2tSVtp', '158Xh48HJR2fr2s2XgCXf2UgmEQPWhBLR', '15DGhFUl7TEdAKj8BmVaWc4Ox4uUNekvz', '15J9bMGOXdsHUBdh4DlFmA_9r7YcgieJT', '15MwO_c4HB1-yJ8WgUG7QhyzOFv1HUOqG', '15Psrrf0dmHIyRcFJiaP4h9V7o3zVM0kc', '15ciocWFrJvZiWsqQ7ZGkBb3nGIxDsw8F', '12wb9urvUQNypsgIs_YW8MtHv_NUuXZ3o', '131mAhHp91hUYkYDpnZ4JHKzEgpONSW_J', '133fZL1NS2QJzKR82UCd49LRkEZPXZu9S', '13Boxp9aeTadWqzm9oWGbRc5kc8o63JNO', '13C2fyoExtwNXbQORLpyH2WsDKOcT0vIe', '13CN3XQDwuahEs6j8RFL8fBG5om0O-y44', '13HPMQG7I--zvHdF2JT2yhc7Z0GsmL1sg', '13OD1T2L_oiiCOiR2I9RNXTX4i38nmFXt', '13VRT5fs7yZqTP4C__0XVK4QMI-mfG6hg', '13cyqLFzD0bwpqU_2mHDOU0-QTGa6TGGU', '13iq0Il1Tb-omDaLiByrRMMyWSvDn3HM9', '13n-EYSwjStHjwAU2pOPaDBJRcfXTm0i8', '13nRRxPzxSxEceIRaLEZv1W7yaFYBUzik', '13t7n1aBkQxO8B1lblA94z31H_1ofttgW', '13vw8V_gYjmVLMYzvrjrDLFEuVDBif65V', '1434xVqT3FB1HW3fXMQVP8aLTiGkWM7QH', '144C-UpJuDD8RVSu86nlsA5rlO6JkWAAW', '14EMinyuaJSpvsD5APrVfxJ_yBrg3Jg0I', '14JSm1GK4NUoDGRNVCGRnXM0LgjZdnP29', '14JlvFIUPYj4aJv8aDBU_tVSrR1iI9QDH', '14LWWfNH-QSsuBF-1jQaB44-2jZXoElAa', '14Nt79kWArKw_aUH7i2KUjk5Y5wVzjUeJ', '14RUverayNZ96CUPNyzw_SzrFTlBpAQjO', '14g-aPP4bU9Aj99Inmdl6y6UthBzlhZNK', '14h6EksZ-LReToMQGou3vO3C2FjDDfoG-', '14hce4vacHRlMQjZNXYSjZtcL-VM3cjlk', '14ilPsvN8HEWkJPy5ks6TU6DJgKn-0QtZ', '14iniyhMGEUEyiJp8nVKFfUpf29gscygE', '14okVb6ILirlj6dUP6Kj6iMDPlgSSyrRa', '14yKm0Q7jvVKSc7DxWh4iWn6nWPYhbK7r', '151CrslMc7RsmWoqFcuk38sC9HUy5BEmR', '151Hn-URMdCdm5h4RBdkfySsyRevUOr-Y', '1599c7w4PSdu3nqRw52hv4UnyekTJ-6w8']
S3_EXTRACT_PATCHES = ['18G_raNO64pcKs5_MqbwpMadbv4uCqEtq', '18ndYBY4liw_Eu66ZOxCdV1F2Xy60GCRc', '19QhENuPZf0S9rbXtgBZu8CTsPT7DjooN', '19RT2p_iesEm4u3X-wgE3ehZVYl6znnYr', '19UF_6SZKpZJXuZ0EI1n-Ou3_GYUR4H4t', '19WeYIAU9dRhknyOtISrxF8gK11UWQMee', '19XD0fXeckOszuHAVy282p3F-OIRtH_BG', '19d3VAQjeCq5-8O5GvuN_EPkQfAcFZVji', '19fGYhGhN78UpdM_M1LKICQppTI85B_2T', '19h2bWRPcYnbrpBmHbi5RDeRcEGAXrVDI', '18MEaVq2wwAWTWF29cXKIiVoRdH8mKykH', '18MzF63C3LOrGKYevAfZcNDlHvTYLOqvT', '18X-i43EWEp70qaMrUcDqhQTXGBv50txc', '18Zh4dCfFtsewUKDHXW15IBnRYWy2UKja', '18_h7yz-Kr8IUVYPzZg3mpL4V1BcfbbMP', '18bPniNDU02bOENDuNsfcljGm2fHBZi4O', '18baPQhHupOPA53U0TuHQq0-XQ54Gt8Hi', '18cVPeDXNUGvS0SupkLXXUpTu9yDxlGqr', '18lUmosn0e6DKDjG7JJP45kqMa2oMh5dV', '18lc-hQtN8iZ5mdtVAL3QJesOJFxlTKr4', '18o03UW9RgX8Sq5vs_q8gI8gNwbCOPwr2', '18r1kNpC2ub78CJn6-AL9K6rpqvGj66Gy', '18upCzBk3-OR5S3KBAGjMhOvfA1uEpndx', '19F6U-h0l5auYo6ALv4_JxX93Bn06Hofx', '19K1Y-0OT8fTPMfnb3myZ_xtmiZmMaU4X', '19MoFlyN35RNVsfI1VTHf8dGkXkZqKWaX', '19jK0AWRd3C5WC3vI5gV2v9MQxc7l6Hnx', '19mMr99rOF-yFst_BIOAF8a0zTv1i0OtX', '19pxp42dfYK371w0irWpV27yTolEzzCHT', '19qrg5FBK52kkM82aTutFhBIIyJSkE3lM', '19rNxcZY3kHX8av08ggV0ewk4oseQZPPW', '19y2eLNDlYCixLCnRicjCoEnwHWvdTNIl', '19zw7IACG-S431_DU-v8a-FSSY1foVMKn', '1A41kKcsltINbM_BpdaoP-iKKolov-y6Q', '1A6oBdTBjd2MqEUCkxfGHpfIirQbik1Uv', '1A9uHrOKvDRBpTvUoe6M3QpW41FUvQ_qX', '1AIy7GYeCg02cAD0OxDs0kL35fwwg579F', '1ALI7dLZ51b69MOSElfI8LR5uUC-9bSBA', '1ARTNGq03sEuNBJX-CWkLPrkQIVxhFlG8', '1AU8atoQILjp4Bp0cEvIlhAHHzWZ-ZQSY', '1AV0KJlEtBFeoRs2wZXbUOhn-FYSe_acq', '1AW8Ybr3cQ8XzuBjzL-MDSVmNiXlXCo--', '1Ag-NiGpPsKeaZKlHH2SGn-CkSfdpapcf']

CREATE_PATCH_FID = S3_CREATE_PATCHES[BATCH_NUMBER]
EXTRACT_PATCH_FID = S3_EXTRACT_PATCHES[BATCH_NUMBER]

!echo -e "[B{BATCH_NUMBER}-S3] Downloading Create Patches Files..."
!curl -H "Authorization: Bearer {ACCESS_TOKEN}" https://www.googleapis.com/drive/v3/files/{CREATE_PATCH_FID}?alt=media -o output_{BATCH_NUMBER}_create_patches.zip

!echo -e "\n[B{BATCH_NUMBER}-S3] Unzipping Create Patches Files..."
!mkdir output_{BATCH_NUMBER}
!unzip -q output_{BATCH_NUMBER}_create_patches.zip -d output_{BATCH_NUMBER}

!echo -e "[B{BATCH_NUMBER}-S3] Downloading Extract Patches Files..."
!curl -H "Authorization: Bearer {ACCESS_TOKEN}" https://www.googleapis.com/drive/v3/files/{EXTRACT_PATCH_FID}?alt=media -o output_{BATCH_NUMBER}_extract_patches.zip

!echo -e "\n[B{BATCH_NUMBER}-S3] Unzipping Extract Patches Files..."
!unzip -q output_{BATCH_NUMBER}_extract_patches.zip -d output_{BATCH_NUMBER}

!echo -e "\n[B{BATCH_NUMBER}-S3] Deleting Extra Files..."
!rm -rf output_{BATCH_NUMBER}_create_patches.zip
!rm -rf output_{BATCH_NUMBER}_extract_patches.zip

!python "prostate-cancer-thesis/scripts/extract_features.py" --encoder "{ENCODER}" --output-base-directory "output_{BATCH_NUMBER}" --dataset-base-directory "prostate-cancer-thesis" --dataset-info-file-name "train.csv" --patch-size 256

!rm -rf output_{BATCH_NUMBER}/create_patches
!rm -rf output_{BATCH_NUMBER}/extract_patches

!echo -e "\n[B{BATCH_NUMBER}-S3] Creating ZIP File For Batch # {BATCH_NUMBER}"
!cd output_{BATCH_NUMBER}; zip -qr -1 "output_{BATCH_NUMBER}.zip" . ; mv "output_{BATCH_NUMBER}.zip" .. ; echo "ZIP File Created For Batch # {BATCH_NUMBER}"

!echo -e "\n[B{BATCH_NUMBER}-S3] Removing Directory For Batch # {BATCH_NUMBER}"
!rm -r "output_{BATCH_NUMBER}"; echo "Directory Removed For Batch # {BATCH_NUMBER}"

!echo -e "\n[B{BATCH_NUMBER}-S3] All Done!"